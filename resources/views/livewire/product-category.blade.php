                    ...
                    <div class="flex flex-col gap-y-10 lg:gap-y-14 xl:gap-y-20">
                        <div class="flex flex-col gap-6">
                            ...
                            @if ($all_products->isNotEmpty())
                                <h5>Kategori Produk: {{ $category->title }}</h5>
                            @else
                                ...
                                @foreach ($all_products as $item)
                                    <div class="jos" data-jos_delay="0">
                                        <div class="group">
                                            ...
                                            <div class="mt-6">
                                                <div
                                                    class="mb-0 flex flex-wrap justify-between gap-5 text-ColorBlack lg:flex-nowrap xl:mb-7">
                                        ...
                                        @if ($item->category->id !== $category->id)
                                            <div style="margin-top: -20px;">
                                                <a class="hover:text-ColorBlue"
                                                    href="/product-category/{{ $item->category->id }}"><small><em>({{ $item->category->title }})</em></small></a>
                                            </div>
                                        @endif
                                ...
                                @foreach ($categories as $item)
                                    @if ($category->id === $item->id)
                                        <li class="pl-8 pb-[10px] last:pb-0 text-ColorBlue">
                                            <a href="/product-category/{{ $item->id }}">{{ $item->title }}</a>
                                        @else
