...
chats_update_channel.bind("chats", function (response) {
    ...
    for (var i = 0; i < response.chats.data.length; i++) {
        if (response.chats.data[i].unseen_messages_count > 0) {
            $("#chat-list").append(
                '<li onclick="fetchData(this)" class="chat-list-item" id="' +
                    response.chats.data[i].id +
                    '" data-sender-name="' +
                    response.chats.data[i].name +
                    '"><span class="chat-list-name">' +
                    response.chats.data[i].name +
                    " (" +
                    response.chats.data[i].email +
                    ')</span><div class="badge">' +
                    response.chats.data[i].unseen_messages_count +
                    "</div>"
            );
        } else {
            $("#chat-list").append(
            ...
            if (response.data[i].unseen_messages > 0) {
                $("#chat-list").append(
                    '<li onclick="fetchData(this)" class="chat-list-item" id="' +
                        ...
                        ')</span><div class="badge">' +
                        response.data[i].unseen_messages +
                        "</div>"
                );
            } else {
                $("#chat-list").append(
